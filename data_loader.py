    ...
    def load_transform_file(self, file_path):
        ...
        with self.driver.session() as session:
            for i in range(len(trips)):
                pickup_node_id = int(trips.iloc[i]['PULocationID'])
                dropoff_node_id = int(trips.iloc[i]['DOLocationID'])
                session.run('''
                                MERGE (pickup:Location {name: $pickup_node_id})
                                MERGE (dropoff:Location {name: $dropoff_node_id})
                            ...
                            distance=float(trips.iloc[i]['trip_distance']),
                            fare=float(trips.iloc[i]['fare_amount']),
                            pickup_dt=trips.iloc[i]['tpep_pickup_datetime'],
                            dropoff_dt=trips.iloc[i]['tpep_dropoff_datetime'])
